//Calculating transverse momentum cross secion RATIOS WITH UNCERTAINTY using the reweighting method, CORRECTLY.
...
double processFile(const std::string &filename, TH1D* hist, int VarNo) 
{
    ...
    for (int i = 0; i < particle_count; ++i)
	{
		...
		if ((pt>30) && ((std::abs(eta) > 2.5) && (std::abs(eta) < 4.5)))
		{
...
int assignment8macro4_1() 
{
    //define histogram binning parameters
    const int nbins = 50;
	const int nbosons =3;
    const double xlow = -5.;
    const double xhigh = 5.;
	const int nPDFs = 3;
	...
	for (std::string PDFID : PDFIDs)
	{
		...
		*/
		...
		TGraphAsymmErrors* RZWUnc = new TGraphAsymmErrors(nbins);
			...
			ApmUnc->GetYaxis()->SetTitle("A_{#pm}(y) ");
			ApmUnc->GetXaxis()->SetTitle("rapidity (y)");

			RpmUnc->GetYaxis()->SetTitle("R_{#pm}(y) ");
			RpmUnc->GetXaxis()->SetTitle("rapidity (y)");

			RZWUnc->GetYaxis()->SetTitle("R_{Z/W}(y) ");
			RZWUnc->GetXaxis()->SetTitle("rapidity (y)");
			...
			for (int bin = 1; bin <= nbins; ++bin)
			{
				...
				if (ratFuncNo==0)
				{
					...
					cout << "Central value of Apm: " << y << " + " << lowerErr << " - " << upperErr <<endl;
				}
				...
				else if (ratFuncNo==1)
				{
					...
					cout << "Central value of Rpm: " << y << " + " << lowerErr << " - " << upperErr <<endl;
				}
				...
				else if (ratFuncNo==2)
				{
					...
					cout << "Central value of RZW: " << y << " + " << lowerErr << " - " << upperErr <<endl;
				}
			...
			if (j==0)
			{
				if (ratFuncNo==0)
				{
					...
					if (j==0)
					{
						...
						texts.DrawLatex(65, .18, "p-p @ #sqrt{s} = 13.6 TeV");
						texts.DrawLatex(65, .16, "POWHEG+PYTHIA8");
						texts.DrawLatex(65, .14, "CT18NNLO");
						texts.DrawLatex(65, .12, "-2.5<y<-4.0");
					}
...
//					if (j==(PDFIDs.size()-1)) leg->Draw("SAME");
				...
				else if (ratFuncNo==1)
				{
					...
					{	RpmUnc->Draw("ASP");
						...
						texts.DrawLatex(10, 1.1, "p-p @ #sqrt{s} = 13.6 TeV");
						texts.DrawLatex(10, 1.0, "POWHEG+PYTHIA8");
						texts.DrawLatex(10, 0.9, "CT18NNLO");
					}
...
//					leg->Draw("SAME");
			...
			if (ratFuncNo==0)
			{
				...
				Apmratio->GetXaxis()->SetTitle("rapidity (y)");
				...
				Apmratio->GetYaxis()->SetTitle("A^{PDF_{i}}_{#pm}(y)/A_{#pm}^{CT18}(y)");
				Apmratio->GetYaxis()->CenterTitle(true);
				Apmratio->GetYaxis()->SetTitleSize(0.04);
...

			{
				...
				Rpmratio->GetXaxis()->SetTitle("rapidity (y)");
				Rpmratio->GetXaxis()->SetTitleSize(0.05);
				Rpmratio->GetYaxis()->SetTitle("R_{#pm}(y) ratio to CT18NNLO");
				Rpmratio->GetYaxis()->CenterTitle(true);
...

			{
				...
				RWZratio->GetXaxis()->SetTitle("rapidity (y)");
				RWZratio->GetYaxis()->SetTitle("R_{Z/W}^{PDF_{i}}(y)/dR_{Z/W}^{CT18}(y)");
				RWZratio->GetYaxis()->CenterTitle(true);
	...
	TFile* outFile =new TFile("NLORatiosMuonyDistComparisionWithScales.root", "RECREATE");
	c1->Write();
	c2->Write();
	c3->Write();
	c4->Write();
