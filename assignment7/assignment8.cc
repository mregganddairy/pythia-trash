...
int main() {
  ...
  int showerModel = pythia.settings.mode("PartonShowers:model");
	...
	vector<vector<double>> eventweights;

	TFile* outFile = new TFile("wp_pwgevents_14000.root", "RECREATE");
  ...
  while (true) {
     ...
     */ 
     ...
     for (int i=0; i < pythia.event.size();++i)
			...
			//getting weight information and storing it in eventweights 2d vector
			double wt = pythia.info.weight();
			vector<double> weights;
			for (int iWt=0; iWt<pythia.info.getInitrwgtSize(); ++iWt) {
				weights.push_back(pythia.info.getWeightsDetailedValue(to_string(iWt)));

				pythia.info.weightContainerPtr->weightsLHEF.bookWeight("VAR"+to_string(iWt),
						pythia.info.getWeightsDetailedValue(to_string(iWt))/wt);
			}
			eventweights.push_back(weights);
  ...
  for (vector<double> testvec: eventweights)
  {
	  for ( double testweight: testvec)
	  {
		  cout << testweight << " ";
	  }
	  cout << endl;
	...
	outFile->WriteObject(&eventweights, "Eventweights");
