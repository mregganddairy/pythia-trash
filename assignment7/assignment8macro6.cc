...
double processFile(const std::string &filename, TH1D* hist, int VarNo) 
{
    ...
    for (int i = 0; i < particle_count; ++i)
	{
		...
		{
			if ((std::abs(eta) > 2.5) && (std::abs(eta) <4.0)){
...
int assignment8macro6() 
{
	...
	for (std::string PDFID : PDFIDs)
	{
		...
		for (int k = 0; k < nbosons; ++k)
		{
			...
			WmUnc->GetYaxis()->SetTitle("d#sigma/dp_{T} (pb/GeV/c)");
			WmUnc->GetXaxis()->SetTitle("p_{T} (pb/GeV/c)");
			WpUnc->GetYaxis()->SetTitle("d#sigma/dp_{T} (pb/GeV/c)");
			ZUnc->GetYaxis()->SetTitle("d#sigma/dp_{T} (pb/GeV/c)");
			...
			for (int bin = 1; bin <= nbins; ++bin)
			{
				...
				if (k==0)
				{
					...
					cout << "Central value of Wm: " << y << " + " << lowerErr << " - " << upperErr <<endl;
				}
				...
				else if (k==1)
				{
					...
					cout << "Central value of Wp: " << y << " + " << lowerErr << " - " << upperErr <<endl;
				}
				...
				else if (k==2)
				{
					...
					Z_Unc[bin-1] = symmErr;
					cout << "Central value of Z: " << y << " + " << lowerErr << " - " << upperErr <<endl;
				}
			...
			if (j==0)
			{
				if (k==0)
				{
					...
					WmUnc->GetYaxis()->SetRangeUser(0., 700.);
					...
					if (j==0)
					{
						...
						texts.DrawLatex(50, 600, "p-p @ #sqrt{s} = 13.6 TeV");
						texts.DrawLatex(55, 300, "POWHEG+PYTHIA8");
						texts.DrawLatex(60, 150, "CT18NNLO");
						texts.DrawLatex(55, 50, "-2.5<y<-4.0");
					}
			...
			if (k==0)
			{
				...
				Wmratio->GetXaxis()->SetTitle("p_{T} (GeV/c)");
				...
				Wmratio->GetYaxis()->SetTitle("#frac{d#sigma_{PDF_{i}}}{dp_{T}}/#frac{d#sigma_{CT18}}{dp_{T}}");
...

			{
				...
				Wpratio->GetXaxis()->SetTitle("p_{T} (GeV/c)");
				Wpratio->GetXaxis()->SetTitleSize(0.05);
				Wpratio->GetYaxis()->SetTitle("#frac{d#sigma_{PDF_{i}}}{dp_{T}}/#frac{d#sigma_{CT18}}{dp_{T}}");
...

			{
				...
				Zratio->GetXaxis()->SetTitle("p_{T} (GeV/c)");
				Zratio->GetYaxis()->SetTitle("#frac{d#sigma_{PDF_{i}}}{dp_{T}}/#frac{d#sigma_{CT18}}{dp_{T}}");
	...
	TFile* outFile =new TFile("NLOMuonPTDistComparisionWithScalesForward_lessbins.root", "RECREATE");
	c1->Write();
	c2->Write();
	c3->Write();
	c4->Write();
