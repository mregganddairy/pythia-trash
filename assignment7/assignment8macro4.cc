...
int assignment8macro4() 
{
	...
	for (std::string PDFID : PDFIDs)
	{
		...
		TLine* MCH1 = new TLine(-4.,0.,-4.,2400.);
		TLine* MCH2 = new TLine(-2.5,0.,-2.5,2400.);

		TLine* CB1 = new TLine(-1.,0.,-1.,2400.);
		TLine* CB2 = new TLine(1,0.,1,2400.);
		...
		for (int k = 0; k < nbosons; ++k)
		{
			...
			WpUnc->GetYaxis()->SetTitle("d#sigma/dy (pb)");
			ZUnc->GetYaxis()->SetTitle("d#sigma/dy (pb)");
			...
			if (j==0)
			{
				if (k==0)
				{
					...
					//WmUnc->SetTitle("W^{-}");
					WmUnc->SetTitle("");
					WmUnc->SetMarkerStyle(markers[k]);
					WmUnc->SetMarkerColor(markercolours[k]);
					WmUnc->SetFillStyle(fills[k]);
					WmUnc->SetLineColor(markercolours[k]);
					WmUnc->SetMarkerSize(0.8);
					WmUnc->GetYaxis()->SetRangeUser(0., 2400.);
					WmUnc->GetXaxis()->SetTitleSize(0.05);
					WmUnc->GetYaxis()->SetTitleSize(0.04);
					...
					if (j==0)
					{
						...
						randomtexts.DrawLatex(-4.0, 1700, "Muon");
						randomtexts.DrawLatex(-4.0, 1620, "Spectrometer");
						randomtexts.DrawLatex(-0.7, 2000, "Central Barrel");
						texts.DrawLatex(-4.5, 2250, "p-p @ #sqrt{s} = 13.6 TeV");
						texts.DrawLatex(-4.5, 2050, "POWHEG+PYTHIA8");
						texts.DrawLatex(-4.5, 1850, "CT18NNLO");

						MCH1->SetLineStyle(kDashed);
						MCH2->SetLineStyle(kDashed);
						MCH1->SetLineColor(kOrange);
						MCH2->SetLineColor(kOrange);

						CB1->SetLineStyle(kDashed);
						CB2->SetLineStyle(kDashed);
						MCH1->SetLineColor(kGreen);
						MCH2->SetLineColor(kGreen);
			...
			if (k==0)
			{
				...
				Wmratio->GetXaxis()->SetTitle("rapidity (y)");
				Wmratio->GetXaxis()->SetTitleSize(0.05);
				Wmratio->GetYaxis()->SetTitleSize(0.04);
				...
				Wmratio->GetYaxis()->SetTitle("#frac{d#sigma_{PDF_{i}}}{dy}/#frac{d#sigma_{CT18}}{dy}");
...

			{
				...
				Wpratio->GetXaxis()->SetTitle("rapidity (y)");
				Wpratio->GetXaxis()->SetTitleSize(0.05);
				Wpratio->GetYaxis()->SetTitle("#frac{d#sigma_{PDF_{i}}}{dy}/#frac{d#sigma_{CT18}}{dy}");
				Wpratio->GetYaxis()->SetTitleSize(0.04);
...

			{
				...
				Zratio->SetMaximum(1.15);
				Zratio->SetMinimum(.85);
				Zratio->SetMarkerSize(0.8);
				Zratio->GetXaxis()->SetRangeUser(xlow, xhigh);
				Zratio->GetXaxis()->SetTitle("rapidity (y)");
				Zratio->GetYaxis()->SetTitle("#frac{d#sigma_{PDF_{i}}}{dy}/#frac{d#sigma_{CT18}}{dy}");
				Zratio->GetXaxis()->SetTitleSize(0.05);
				Zratio->GetYaxis()->SetTitleSize(0.04);
	...
	TFile* outFile =new TFile("NLOMuonEtaDistComparisionWithScales.root", "RECREATE");
	c1->Write();
	c2->Write();
	c3->Write();
	c4->Write();
